...
- `SS_REMOTE_HOST` + `SS_REMOTE_PORT` -> recursive resolver fallback (used only when no
  `--resolver`/`--dev-authoritative` values are provided via CLI or plugin options; see
...
`keep-alive-interval`. The `authoritative` SIP003 option maps to the client’s development-only
direct authoritative mode. Server consumes `domain`, `cert`, `key`, `reset-seed`, `fallback`,
and `max-connections`.
